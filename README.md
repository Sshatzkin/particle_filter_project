...
- initialize_particle_cloud: We will initialize our particle cloud by mapping our environment with SLAM initially. Afterwards, we will place an extremely large amount of particles throughout this environment, essentially "filling" this space with particles which we can then use to estimate the robot's position later.
...
- update_estimated_robot_pose: Then, we will update the estimated pose of the robot 
